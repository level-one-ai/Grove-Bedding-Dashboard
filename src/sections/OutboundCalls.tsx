...
const callLogs: CallLog[] = [
  {
    id: 'CALL-001', time: '09:30', date: '24 Mar 2026',
    contactName: 'Acme Furniture Co', contactPerson: 'James Wilson',
    phone: '+44 121 555 0100', duration: '4m 32s', orderId: 'SO-1021',
    status: 'completed', outcome: 'Order confirmed & dispatch arranged',
    transcript: [
      ...
      { speaker: 'Agent', text: 'No issues at all. Your order has been processed and is scheduled for dispatch today. Delivery is estimated for tomorrow between 9am and 12pm.', time: '09:30:42' },
      ...
      { speaker: 'Agent', text: 'You are welcome. Have a great day.', time: '09:31:38' },
    ],
  },
  {
    id: 'CALL-002', time: '09:52', date: '24 Mar 2026',
    contactName: 'SleepWell Retailers', contactPerson: 'Sarah Thompson',
    phone: '+44 161 555 0200', duration: '2m 18s', orderId: 'SO-1020',
    status: 'completed', outcome: 'Delivery window confirmed',
    transcript: [
      ...
      { speaker: 'Agent', text: 'Thank you Sarah. Have a great day.', time: '09:53:20' },
    ],
  },
  {
    id: 'CALL-003', time: '10:15', date: '24 Mar 2026',
    contactName: 'Comfort Home Store', contactPerson: 'N/A',
    phone: '+44 113 555 0300', duration: '0m 45s', orderId: 'SO-1019',
    status: 'no-answer', outcome: 'No answer — voicemail left',
    transcript: [
  ...
  {
    id: 'CALL-004', time: '10:45', date: '24 Mar 2026',
    contactName: 'Dream Sleep Outlet', contactPerson: 'Michael Chen',
    phone: '+44 114 555 0400', duration: '6m 04s', orderId: 'SO-1018',
    status: 'completed', outcome: 'Order amended — quantity updated',
    transcript: [
      ...
      { speaker: 'Agent', text: 'I am calling about your order for 15 units of Down Alternative Pillows. Would you like to amend the quantity before dispatch?', time: '10:45:14' },
      { speaker: 'Contact', text: 'Actually yes — can we increase the pillow order to 25 units?', time: '10:45:50' },
      { speaker: 'Agent', text: 'Of course. I will update order SO-1018 to 25 units. The additional 10 units will be included in the same dispatch at no extra delivery charge.', time: '10:45:57' },
      { speaker: 'Contact', text: 'That is great, thank you. Very efficient service as always.', time: '10:46:20' },
      { speaker: 'Agent', text: 'Thank you Michael. Your updated order confirmation will be emailed within the next few minutes.', time: '10:46:26' },
    ],
  },
  {
    id: 'CALL-005', time: '11:30', date: '24 Mar 2026',
    contactName: 'BedCraft Interiors', contactPerson: 'N/A',
    phone: '+44 207 555 0500', duration: '0m 45s', orderId: 'SO-1017',
    status: 'failed', outcome: 'Call failed — connection error',
    transcript: [
  ...
  {
    id: 'CALL-006', time: '13:15', date: '24 Mar 2026',
    contactName: 'Nordic Sleep Co', contactPerson: 'Anna Johansson',
    phone: '+44 121 555 0600', duration: '3m 22s', orderId: 'SO-1016',
    status: 'completed', outcome: 'Delivery instructions updated',
    transcript: [
      ...
      { speaker: 'Contact', text: 'Hello, Anna Johansson speaking.', time: '13:15:12' },
      { speaker: 'Agent', text: 'Good afternoon Anna. I am calling to confirm delivery of 20 Luxury Bed Sheet sets. Can you confirm the delivery address is still correct?', time: '13:15:18' },
      ...
      { speaker: 'Agent', text: 'Thank you for letting us know. I have updated the delivery address to Unit 8. Delivery is still scheduled for this Thursday.', time: '13:15:52' },
      { speaker: 'Contact', text: 'Brilliant, thank you for checking.', time: '13:16:20' },
      { speaker: 'Agent', text: 'No problem at all. You will receive an updated confirmation shortly.', time: '13:16:28' },
    ],
  },
  {
    id: 'CALL-007', time: '14:00', date: '23 Mar 2026',
    contactName: 'Pillow Palace Ltd', contactPerson: 'Robert Davies',
    phone: '+44 29 555 0700', duration: '5m 10s', orderId: 'SO-1015',
    status: 'completed', outcome: 'Order confirmed — express upgrade requested',
    transcript: [
      ...
      { speaker: 'Agent', text: 'Your order for 30 Down Alternative Pillows is ready for dispatch. Would you like to upgrade to next day delivery?', time: '14:00:15' },
      { speaker: 'Contact', text: 'Yes please, we need them urgently. What is the cost?', time: '14:00:50' },
      { speaker: 'Agent', text: 'Next day delivery is available at an additional charge of £15. Shall I arrange that?', time: '14:01:00' },
      { speaker: 'Contact', text: 'Yes, go ahead please.', time: '14:01:15' },
      { speaker: 'Agent', text: 'Excellent. Your order will be dispatched this evening for delivery tomorrow before 12pm.', time: '14:01:20' },
    ],
  },
];
...
export default function OutboundCalls() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo('.call-card', { y: 10, opacity: 0 }, { y: 0, opacity: 1, duration: 0.35, stagger: 0.06, ease: 'power2.out' });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <div ref={sectionRef} className="h-full overflow-hidden flex flex-col" style={{ background: '#f8fafc' }}>
      <div className="flex-1 flex flex-col min-h-0 max-w-[700px] w-full mx-auto px-6 py-4">

        {/* Header — fixed */}
        <div className="flex-shrink-0 mb-4 flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="font-sora font-bold text-xl" style={{ color: '#1e293b' }}>Outbound Calls</h1>
            <p className="font-inter text-xs mt-0.5" style={{ color: '#94a3b8' }}>Automated call agent · Click any call to view transcript</p>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 px-4 py-3">
                  {/* Status icon */}
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: cfg.bg, border: `1px solid ${cfg.bo}` }}>
                    <StatusIcon className="w-4 h-4" style={{ color: cfg.color }} />
                  </div>

                  {/* Date + time */}
                  <div className="flex-shrink-0 text-center w-16">
                    <p className="font-sora font-semibold text-sm" style={{ color: '#1e293b' }}>{call.time}</p>
                    <p className="font-mono text-[9px]" style={{ color: '#94a3b8' }}>{call.date.split(' ').slice(0, 2).join(' ')}</p>
                  </div>

                  {/* Divider */}
                  <div className="w-px h-8 flex-shrink-0" style={{ background: '#f1f5f9' }} />

                  {/* Contact info */}
                  <div className="flex-1 min-w-0">
                    <p className="font-sora font-semibold text-sm group-hover:text-sky-600 transition-colors truncate" style={{ color: '#1e293b' }}>{call.contactName}</p>
                    <p className="font-inter text-xs truncate" style={{ color: '#94a3b8' }}>{call.outcome}</p>
                  </div>

                  {/* Right side */}
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className="font-mono text-xs" style={{ color: '#0ea5e9' }}>{call.orderId}</span>
                    <div className="px-2 py-0.5 rounded-full" style={{ background: cfg.bg, border: `1px solid ${cfg.bo}` }}>
                      <span className="font-sora text-[10px] font-semibold" style={{ color: cfg.color }}>{cfg.label}</span>
                    </div>
      ...
      {/* Dialog — fixed position, scrollable body */}
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl" style={{ background: '#ffffff', border: '1px solid #e2e8f0', maxHeight: '80vh', display: 'flex', flexDirection: 'column', position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', margin: 0 }}>
          {selectedCall && (() => {
                    ...
                    <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #e2e8f0' }}>
                      {selectedCall.transcript.map((line, idx) => (
                        <div key={idx} className="flex gap-3 px-4 py-3"
                          style={{ background: line.speaker === 'Agent' ? '#f8fafc' : '#ffffff', borderTop: idx > 0 ? '1px solid #f1f5f9' : 'none' }}>
                          <div className="flex-shrink-0 w-14">
                            <span className="font-mono text-[9px] uppercase tracking-wide font-bold block" style={{ color: line.speaker === 'Agent' ? '#0ea5e9' : '#64748b' }}>{line.speaker}</span>
                            {line.time && <span className="font-mono text-[9px] block mt-0.5" style={{ color: '#cbd5e1' }}>{line.time.slice(6)}</span>}
                          </div>
                          <p className="font-inter text-xs leading-relaxed flex-1" style={{ color: '#334155' }}>{line.text}</p>
                        </div>
                      ))}
                    </div>
                  </div>
