...
const automations: Automation[] = [
  {
    ...
    runsThisMonth: 247, failuresThisMonth: 3,
    lastRun: '09:14 today', lastStatus: 'success',
    recentRuns: [
  ...
  },
  {
    ...
    icon: FileText,
    color: '#8b5cf6', bg: '#faf5ff', border: '#e9d5ff',
    runsThisMonth: 89, failuresThisMonth: 1,
    lastRun: '11:32 today', lastStatus: 'success',
    recentRuns: [
  ...
  },
  {
    ...
    runsThisMonth: 187, failuresThisMonth: 4,
    lastRun: '13:44 today', lastStatus: 'success',
    recentRuns: [
  ...
  },
  {
    ...
    runsThisMonth: 156, failuresThisMonth: 0,
    lastRun: '14:10 today', lastStatus: 'success',
    recentRuns: [
  ...
  },
  {
    ...
    runsThisMonth: 312, failuresThisMonth: 2,
    lastRun: '11:34 today', lastStatus: 'success',
    recentRuns: [
  ...
  },
  {
    ...
    runsThisMonth: 74, failuresThisMonth: 6,
    lastRun: '13:15 today', lastStatus: 'success',
    recentRuns: [
...
export default function AutomationStream() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo('.auto-row', { x: -10, opacity: 0 }, { x: 0, opacity: 1, duration: 0.4, stagger: 0.05, ease: 'power2.out' });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <div ref={sectionRef} className="h-full overflow-hidden flex flex-col" style={{ background: '#f8fafc' }}>
      <div className="flex-1 flex flex-col min-h-0 max-w-[1000px] w-full mx-auto px-6 py-4">

        {/* Header — fixed */}
        <div className="flex-shrink-0 mb-4 flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="font-sora font-bold text-xl" style={{ color: '#1e293b' }}>Automations</h1>
            <p className="font-inter text-xs mt-0.5" style={{ color: '#94a3b8' }}>All automations · Resets 1st of each month</p>
          </div>
        ...
        <div className="flex-1 min-h-0 rounded-2xl overflow-hidden" style={{ background: '#ffffff', border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <div className="h-full overflow-y-auto">
            {automations.map((auto, idx) => {
              const Icon = auto.icon;
              const statusDot = { success: '#10b981', failed: '#ef4444', warning: '#f59e0b', idle: '#94a3b8' }[auto.lastStatus];
              return (
                <div
                  key={auto.id}
                  className="auto-row flex items-center gap-4 px-5 py-3.5 cursor-pointer hover:bg-slate-50 transition-all duration-200 group"
                  style={{ borderBottom: idx < automations.length - 1 ? '1px solid #f1f5f9' : 'none' }}
                  onClick={() => { setSelected(auto); setDialogOpen(true); }}
                >
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: auto.bg, border: `1px solid ${auto.border}` }}>
                    <Icon className="w-4 h-4" style={{ color: auto.color }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-sora font-semibold text-sm group-hover:text-sky-600 transition-colors" style={{ color: '#1e293b' }}>{auto.name}</p>
                    <p className="font-inter text-xs truncate" style={{ color: '#94a3b8' }}>{auto.description}</p>
                  </div>
                  <div className="flex items-center gap-5 flex-shrink-0">
                    <div className="text-center">
                      <p className="font-sora font-bold text-base" style={{ color: '#1e293b' }}>{auto.runsThisMonth}</p>
                      <p className="font-mono text-[9px] uppercase" style={{ color: '#94a3b8' }}>runs</p>
                    </div>
                    <div className="text-center">
                      <p className="font-sora font-bold text-base" style={{ color: auto.failuresThisMonth > 0 ? '#ef4444' : '#10b981' }}>{auto.failuresThisMonth}</p>
                      <p className="font-mono text-[9px] uppercase" style={{ color: '#94a3b8' }}>failed</p>
                    </div>
              ...
              <>
                <DialogHeader className="flex-shrink-0 pb-4" style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <DialogTitle className="font-sora font-bold text-lg flex items-center gap-3" style={{ color: '#1e293b' }}>
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: selected.bg, border: `1px solid ${selected.border}` }}>
                ...
                <div className="mt-4 flex-1 min-h-0 overflow-hidden flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto rounded-xl" style={{ border: '1px solid #f1f5f9' }}>
